...
export function useAgeAssuranceState(): AgeAssuranceState {
  // GHOST: patched to always report full, assured access. This is the single
  // source of truth read by both the hard block in view/shell/index(.web).tsx
  // (`aa.state.access === aa.Access.None`) and the softer restriction flags
  // computed in ageAssurance/util.ts (`isAgeRestricted`), so patching here
